...
from .const import DOMAIN, CONF_VALUE, DEFAULT_VALUE, DEVICES, DEFAULT_PORT
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    async def async_step_user(self, user_input=None, error=None):
        if user_input is not None:
            if DOMAIN in self.hass.data and DEVICES in \
                    self.hass.data[DOMAIN] and user_input[CONF_HOST] in \
